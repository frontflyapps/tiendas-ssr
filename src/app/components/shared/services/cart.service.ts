...
export class CartService implements OnDestroy {
  ...
  private async checkIfUserWantToRequestProduct() {
    ...
    const wantsToContinue = await dialogRef.afterClosed().toPromise();
    if (wantsToContinue) {
      return true;
    }
    return false;
    ...
    if (product.type != 'physical') {
      if (this._isInCart(product)) {
        message =
          this.translate.instant('The product ') +
          productName +
          this.translate.instant(' it is already in the cart.');
        status = 'error';
  ...
  async registerData() {
    if (this.loggedInUser) {
      try {
        const localStorageCarts: Cart[] =
          this.loggedInUserService._getDataFromStorage('cartItem') || [];
        for (const cart of localStorageCarts) {
