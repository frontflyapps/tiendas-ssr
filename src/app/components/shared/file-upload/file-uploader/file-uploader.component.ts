...
export class FileUploaderComponent implements OnInit, OnDestroy {
  ...
  subscriptionsForm() {
    this.form.get('formatType').valueChanges.subscribe((change) => {
  ...
  handleFileSelect(event) {
    ...
    if (file.type.split('/')[0] === 'video' && !typesSelected.includes('video/*')) {
      ...
      this.showToastr.showError(
        'El archivo seleccionado no está permitdo, solo se puede subir archivos de tipo' +
          typesSelected,
      );
      return;
    }
