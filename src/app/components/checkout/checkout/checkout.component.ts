...
export class CheckoutComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.activateRoute.queryParams.subscribe((data) => {
      ...
      this.cartItemIds =
        this.cartItemIds && this.cartItemIds.constructor != Array
          ? [this.cartItemIds]
          : this.cartItemIds;
      this.processToCart();
    });
    ...
    this.cartService.getCartData({ cartId: this.cartId, cartItemIds: this.cartItemIds }).subscribe({
      next: (data) => {
        ...
        if (data.CartItems.length === 0) {
          ...
          this.marketCard =
            this.buyProducts && this.buyProducts.length > 0
              ? this.buyProducts[0].Product.market
              : MarketEnum.NATIONAL;
  ...
  fetchData() {
    // Get Shipping
    const dataCartId = { cartId: this.cartId };
    this.inLoading = true;
    ...
    this.configurationService.getCustomFields(dataCartId).subscribe((data) => {
      ...
      if (this.customFields?.length > 0) {
        for (const val of this.customFields) {
  ...
  onPayOrder() {
    ...
    if (
      this.form.get('ShippingBusinessId').value &&
      typeof this.form.get('ShippingBusinessId').value === 'object'
    ) {
      this.form
        .get('ShippingBusinessId')
        .setValue(this.form.get('ShippingBusinessId').value.BusinessId);
    }
    ...
    this.payService.makePaymentTransfermovil(bodyData).subscribe({
      next: (data: any) => {
        if (data && data.data) {
          ...
          const dialogRef = this.dialog.open(DialogTranfermovilQrComponent, {
            width: '50rem',
  ...
  processBidaiondo(bodyData) {
    ...
    if (bodyData.paymentType === 'peoplegoto') {
      ...
      paymentMethod.subscribe(
        (data: any) => {
          const dialogRef = this.dialog.open(DialogBidaiondoConfirmToPayComponent, {
            width: '15cm',
            maxWidth: '100vw',
            data: {
              form: data.data.form,
  ...
  onSelectContact(contact) {
    ...
    this.form
      .get('address')
      .get('street')
      .setValue(contact?.address.street);
    this.form
      .get('address')
      .get('number')
      .setValue(contact?.address.number);
    this.form
      .get('address')
      .get('between')
      .setValue(contact?.address.between);
