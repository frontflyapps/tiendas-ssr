...
export class DialogPrescriptionComponent implements OnInit {
  ...
  onChangeSelection(
    ...
    this.supplementArray[positionArrayFather].Recomendeds.forEach((item) => {
      if (
        ...
        this.supplementArray[positionArrayFather].Recomendeds[positionArrayChild].RecomendedProduct
          .name.es
      ) {
        item.checked = false;
      } else {
        if (positionArrayFather === 0) {
          ...
          this.supplementArray[3].Recomendeds.forEach((item2) => {
            item2.checked = false;
          });
          ...
          this.supplementArray[1].Recomendeds.forEach((item2) => {
            item2.checked = false;
          });
          this.supplementArray[2].Recomendeds.forEach((item2) => {
            item2.checked = false;
          });
        }
        item.checked = true;
  ...
  buyNow() {
    ...
    if (this.supplementForm.get('supplementType').value && this.form.value) {
      supplements.push(
        this.supplementForm.get('supplementType').value.RecomendedProduct.Stocks[0].uuid,
      );
      if (this.supplementForm.get('supplementDye').value) {
        supplements.push(
          this.supplementForm.get('supplementDye').value.RecomendedProduct.Stocks[0].uuid,
        );
      }
      if (this.supplementForm.get('supplementColor').value) {
        supplements.push(
          this.supplementForm.get('supplementColor').value.RecomendedProduct.Stocks[0].uuid,
        );
      }
  ...
  save() {
    ...
    if (this.supplementForm.get('supplementType').value && this.form.value) {
      ...
      const dataToSend = {
        StockId: this.data.product.Stock.id,
        ProductId: this.data.product.Stock.id,
        supplementIds: this.supplementForm
          .get('supplement')
          .value.map((item) => item.RecomendedProduct.Stocks[0].uuid),
        prescription: this.form.value,
      };
